...
const icon = (name: "arrow" | "check" | "download" | "github" | "globe" | "history" | "image" | "refresh" | "shield" | "windows" | "android") => {
  const paths = {
    arrow: '<path d="M5 12h14M13 6l6 6-6 6"/>',
    check: '<path d="m5 12 4 4L19 6"/>',
    download: '<path d="M12 3v12m0 0 5-5m-5 5-5-5M5 21h14"/>',
    github: '<path fill="currentColor" stroke="none" d="M12 2a10 10 0 0 0-3.16 19.49c.5.09.68-.22.68-.48v-1.69c-2.78.6-3.37-1.18-3.37-1.18-.45-1.16-1.11-1.47-1.11-1.47-.91-.62.07-.61.07-.61 1 .07 1.53 1.03 1.53 1.03.9 1.53 2.35 1.09 2.92.83.09-.65.35-1.09.64-1.34-2.22-.25-4.56-1.11-4.56-4.94 0-1.09.39-1.98 1.03-2.68-.1-.25-.45-1.27.1-2.64 0 0 .84-.27 2.75 1.02A9.56 9.56 0 0 1 12 6.7c.85 0 1.71.11 2.51.34 1.91-1.29 2.75-1.02 2.75-1.02.55 1.37.2 2.39.1 2.64.64.7 1.03 1.59 1.03 2.68 0 3.84-2.34 4.68-4.57 4.93.36.31.68.92.68 1.86v2.76c0 .27.18.58.69.48A10 10 0 0 0 12 2Z"/>',
...
export function landingPage(userAgent = ""): string {
  ...
  const arm64Portable = `LuodongChat-${version}-win-arm64-portable.zip`;
  const downloads = "https://oss.520skx.com/latest";
  const isAndroid = /android/i.test(userAgent);
  const primary = isAndroid
    ? { label: "下载 Android 稳定版", meta: "Android 10+ · 暂停维护", href: "https://oss.520skx.com/latest/LuodongChat.apk", file: "LuodongChat.apk", platform: "android" as const }
    : { label: "下载 Windows x64 版", meta: `v${version} · Windows 10/11 x64`, href: `${downloads}/${x64Setup}`, file: x64Setup, platform: "windows" as const };
  const secondary = isAndroid
    ? { label: "Windows x64 版", href: `${downloads}/${x64Setup}`, file: x64Setup, platform: "windows" as const }
    : { label: "Android 稳定版", href: "https://oss.520skx.com/latest/LuodongChat.apk", file: "LuodongChat.apk", platform: "android" as const };
...
<body>
  <main class="page-shell">
    <header class="topbar">
      ...
      <nav class="topnav" aria-label="主导航"><a href="#features">功能</a><a href="#privacy">安全</a><a class="github-link" href="https://github.com/SunKeXu01/LuodongChat" aria-label="访问泺栋 Chat GitHub 项目">${icon("github")}<span>GitHub</span></a><a href="/healthz">运行状态 <span class="status-dot"></span></a></nav>
    </header>
    ...
    <section class="hero">
      <div class="hero-copy">
        ...
        <div class="quick-links"><a href="${downloads}/${arm64Setup}" data-download data-file="${arm64Setup}">Windows ARM64</a><span>·</span><a href="${downloads}/${x64Portable}" data-download data-file="${x64Portable}">x64 便携版</a><span>·</span><a href="https://github.com/SunKeXu01/LuodongChat/releases/latest">历史版本</a><span class="download-feedback" aria-live="polite"></span></div>
        <p class="install-note">Apple 芯片虚拟机及 Windows ARM 设备请选择 ARM64 版。安装包暂未代码签名，首次运行可能出现系统保护提示。<a href="https://github.com/SunKeXu01/LuodongChat#Windows-使用方式">查看安装指引 →</a></p>
      </div>
    ...
    <section class="download-details" id="downloads">
      ...
      <div class="download-links"><a href="${downloads}/${x64Setup}" data-download data-file="${x64Setup}">Windows x64 安装版</a><a href="${downloads}/${arm64Setup}" data-download data-file="${arm64Setup}">Windows ARM64 安装版</a><a href="${downloads}/${x64Portable}" data-download data-file="${x64Portable}">x64 便携版</a><a href="${downloads}/${arm64Portable}" data-download data-file="${arm64Portable}">ARM64 便携版</a><a href="https://oss.520skx.com/latest/LuodongChat.apk" data-download data-file="LuodongChat.apk">Android 稳定版</a><a href="${downloads}/${arm64Setup}.sha256">ARM64 SHA-256</a></div>
    </section>
    ...
    <footer class="footer"><nav><a href="https://github.com/SunKeXu01/LuodongChat/blob/main/docs/DATA_MODEL.md">隐私说明</a><a href="https://github.com/SunKeXu01/LuodongChat#Windows-使用方式">安装帮助</a><a href="https://github.com/SunKeXu01/LuodongChat/releases/latest">更新记录</a><a href="https://github.com/SunKeXu01/LuodongChat/issues">联系我们</a></nav><span>v${version} · 服务正常 <i class="status-dot"></i></span></footer>
  </main>
...
.page-shell{width:min(1180px,calc(100% - 32px));margin:20px auto;padding:0 32px 20px;border:1px solid var(--line);border-radius:24px;background:var(--surface);box-shadow:0 12px 34px #14203309}.topbar{height:68px;display:flex;align-items:center;justify-content:space-between;border-bottom:1px solid var(--line)}.brand{display:flex;align-items:center;gap:10px;text-decoration:none;font-size:18px;font-weight:750;letter-spacing:-.02em}.brand-mark,.window-logo{display:grid;place-items:center;background:var(--brand);color:#6ee7dc;font:800 14px/1 ui-monospace,SFMono-Regular,Menlo,monospace;letter-spacing:-1px}.brand-mark{width:36px;height:36px;border-radius:11px}.topnav{display:flex;align-items:center;gap:20px;color:var(--muted);font-size:13px;font-weight:600}.topnav a{display:flex;align-items:center;gap:8px;text-decoration:none}.topnav a:hover{color:var(--ink)}.topnav .github-link{min-height:38px;padding:0 13px;border:1px solid #142033;border-radius:11px;background:#142033;color:#fff;font-size:13px;font-weight:750;box-shadow:0 5px 12px #14203318;transition:transform .18s ease,background .18s ease,box-shadow .18s ease}.topnav .github-link .icon{width:19px;height:19px}.topnav .github-link:hover{background:#253349;color:#fff;box-shadow:0 7px 16px #14203324;transform:translateY(-1px)}.topnav .github-link:active{transform:translateY(0)}.status-dot{display:inline-block;width:7px;height:7px;border-radius:50%;background:var(--accent);box-shadow:0 0 0 3px #12b88618}
...
@media(max-width:430px){.topnav a:nth-child(4){display:none}.topnav .github-link{min-height:36px;padding-inline:11px}.hero{padding-top:26px}h1{font-size:32px}.features{grid-template-columns:1fr}.features article+article{border-left:0;border-top:1px solid var(--line)}.app-sidebar{display:none}.app-layout{grid-template-columns:1fr}.product-stage{margin-inline:-4px}.window-controls{font-size:8px}.lead{line-height:1.65}}
