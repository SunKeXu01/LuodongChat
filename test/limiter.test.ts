...
test("enforces concurrency and releases permits", () => {
  ...
  assert.deepEqual(limiter.acquire("user", 1), { ok: false, reason: "concurrency" });
...
test("enforces the per-minute request limit", () => {
  ...
  const first = limiter.acquire("user", 0);
  if (first.ok) first.release();
  assert.deepEqual(limiter.acquire("user", 1), { ok: false, reason: "rate" });
  assert.equal(limiter.acquire("user", 60_001).ok, true);
});
