...
The public health endpoint is `https://luodongchat.com/healthz`. On the server:
...
Set `ADMIN_KEY_HASHES` to one or more SHA-256 hashes of dedicated administrator keys, then recreate the gateway container. Open `https://luodongchat.com/admin` and enter the corresponding plaintext administrator key. Do not reuse a user gateway key.
