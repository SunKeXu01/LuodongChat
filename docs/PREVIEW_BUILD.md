...
当前 Windows 版本为 `1.15`，目标平台为 Windows 10/11 x64，采用 .NET 10 自包含单文件发布；Android 暂停维护，仅在手动触发流水线时构建。
...
```powershell
$expected = (Get-Content .\LuodongChat-1.15-win-x64-portable.zip.sha256).Trim()
.\deploy\Test-PreviewBuild.ps1 `
  -ZipPath .\LuodongChat-1.15-win-x64-portable.zip `
