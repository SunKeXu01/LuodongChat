...
当前版本为 `1.1.1`，Windows 目标平台为 Windows 10/11 x64，采用 .NET 10 自包含单文件发布；Android 目标为 Android 10 及以上。
...
```powershell
$expected = (Get-Content .\LuodongChat-1.1.1-win-x64-portable.zip.sha256).Trim()
.\deploy\Test-PreviewBuild.ps1 `
  -ZipPath .\LuodongChat-1.1.1-win-x64-portable.zip `
