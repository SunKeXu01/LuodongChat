...
当前版本为 `0.1.0-preview.8`，目标平台为 Windows 10/11 x64，采用 .NET 10 自包含单文件发布。
...
```powershell
$expected = (Get-Content .\ChatGPTConnector-0.1.0-preview.8-win-x64.zip.sha256).Trim()
.\deploy\Test-PreviewBuild.ps1 `
  -ZipPath .\ChatGPTConnector-0.1.0-preview.8-win-x64.zip `
