...
- 支持 MCP：可连接本地 stdio 或远程 Streamable HTTP/SSE 服务器，默认由 AI 自动选择已启用工具；公开只读查询可自动执行，敏感读取、写入、外部操作及高风险动作受权限确认控制
...
GitHub Release 和 OSS 文件均包含版本号，例如 `LuodongChat-2.0.10-win-x64-setup.exe`、`LuodongChat-2.0.10-win-arm64-setup.exe` 及各自的便携 ZIP，并同时提供 SHA-256 校验文件。OSS 还保留不带版本号的固定地址用于自动更新，并且只存储最新版，不积累历史安装包。客户端会根据自身架构自动选择后续更新。Android 当前暂停维护，保留最后一个稳定版的固定下载地址。
