...
- Windows 支持项目空间：GPT 可读取和搜索项目文件，在用户逐次确认后创建、修改、移动或删除文件，并可运行构建、测试、格式化和静态检查命令
...
6. 点击“新建对话”可以选择普通对话或项目空间；选择本地项目目录后，该会话会永久归入对应项目。读取和文件名搜索可自动执行；创建、覆盖、替换、移动、删除或运行命令前，客户端会展示具体路径或完整命令并逐次询问。文件工具只能访问选中的项目目录，默认排除密钥文件、版本库元数据、依赖目录、构建产物、符号链接和目录联接。命令工具用于构建、测试、格式化、静态检查和查看版本状态，不静默提权，不继承 API 密钥环境变量，并阻止关机、磁盘、账户、服务、注册表、删除、远程访问及发布命令；命令不是完整沙箱，用户仍应在确认前核对内容。
...
聊天交互设计参考了 [NextChat](https://github.com/ChatGPTNextWeb/NextChat)、[DeepChat](https://github.com/ThinkInAIXYZ/deepchat) 与 [Chatbox](https://github.com/chatboxai/chatbox) 的产品思路；皮肤状态管理参考了 MIT 许可的 [Codex Dream Skin](https://github.com/Fei-Away/Codex-Dream-Skin)；项目文件工具的工作区隔离、读写分权和工具循环设计参考了 MIT 许可的 [OpenClaw](https://github.com/openclaw/openclaw)。泺栋 Chat 使用 WPF 与 C# 独立实现这些能力，不捆绑 OpenClaw 的 Node.js 网关，也不复制外部项目的受限素材。
