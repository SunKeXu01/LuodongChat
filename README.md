...
适用于 Windows 10/11 x64 的 GPT-5.6 连接客户端。用户通过邮箱验证码注册或登录，无需手动领取或填写网关密钥。
...
- 不直接修改用户原来的 `.codex/config.toml` 和 `auth.json`
...
Release 中的 EXE、ZIP 和校验文件均包含版本号，例如 `ChatGPTConnector-0.1.0-preview.10-win-x64.exe`。
...
3. 输入验证码并点击“登录 / 注册”。
4. 登录成功后，连接器自动创建独立的托管 Codex 环境并启动本地代理。
5. 重新启动 ChatGPT 或 Codex，使新的 `CODEX_HOME` 生效。
6. 使用期间保持连接器运行并处于登录状态，可以最小化到系统托盘。

关闭窗口右上角叉号只会隐藏到系统托盘。需要完全退出时，右键托盘图标并选择“退出”。完全退出或退出登录后，连接器会恢复此前的 `CODEX_HOME`；如果用户原来没有 `.codex` 目录，也不会替用户创建或修改原始配置目录。
