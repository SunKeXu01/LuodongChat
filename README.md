...
GitHub Release 和 OSS 文件均包含版本号，例如 `LuodongChat-1.12-win-x64-setup.exe`、`LuodongChat-1.12-win-x64-portable.zip` 与 `LuodongChat-1.12-android.apk`，并同时提供 SHA-256 校验文件。OSS 还保留不带版本号的固定地址用于自动更新，并且只存储最新版，不积累历史安装包。
