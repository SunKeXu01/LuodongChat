...
GitHub Release 和 OSS 文件均包含版本号，例如 `LuodongChat-1.27-win-x64-setup.exe`、`LuodongChat-1.27-win-arm64-setup.exe` 及各自的便携 ZIP，并同时提供 SHA-256 校验文件。OSS 还保留不带版本号的固定地址用于自动更新，并且只存储最新版，不积累历史安装包。客户端会根据自身架构自动选择后续更新。Android 当前暂停维护，保留最后一个稳定版的固定下载地址。
...
7. 点击输入框左下角回形针可一次选择多个附件，也可把文件拖到聊天页面任意位置，或在输入框聚焦时按 Ctrl+V 直接粘贴截图。单个文件最大 20 MB，每条消息最多 10 个；服务器临时附件 30 分钟后自动删除，已发送附件仅随本地对话保存在当前安装目录。
